...
def charLink(char):
    return "<a href='%s'>%s</a>" % (char.attrib["cp"] + ".html",
                                    "U+" + char.attrib["cp"])
...
for _, char in chars.items():
    ...
    props = [
        ("Block", "<a href='%s'>%s</a>" %
         ("b" + block.attrib["first-cp"] + ".html", block.attrib["name"])),
    ...
    if char.attrib.get("dm", "#") != "#":
        ...
        if None in dms:
            ...
            props.append(("Decomposition", " ".join([
                "<a href='%s'>%s</a>" %
                (dm.attrib["cp"] + ".html", charTitle(dm)) for dm in dms
            ])))
    ...
    for char in range(first, last + 1):
        if char in chars:
            ...
            overviewChars.append(
                "<a href='%s'>%s</a>" %
                (chars[char].attrib["cp"] + ".html", htmlOverviewChar))
            blockChars.append("<li><i>%s</i> <a href='%s'>%s</a></i>" %
                              (htmlChar, chars[char].attrib["cp"] + ".html",
                               charTitle(chars[char])))
    with open(os.path.join(generatedDocsDir, docFile), "w") as f:
        f.write(
...
""" % {
            ...
            "".join([
                ...
                ("c/b" + b.attrib["first-cp"] + ".html", b.attrib["name"],
                 b.attrib["first-cp"], b.attrib["last-cp"]) for b in blocks
            ])
        })
        ...
        json.dumps(
            {
                ...
                "Unicode Characters",
                "version":
                "%s/%s" % (version, datetime.datetime.utcnow().isoformat()),
                "archive":
                "UnicodeCharacters.tgz",
                "author": {
                    "name": "Remko Tronçon",
