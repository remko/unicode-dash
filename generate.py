...
chars = {}
for char in root.findall("{http://www.unicode.org/ns/2003/ucd/1.0}repertoire/{http://www.unicode.org/ns/2003/ucd/1.0}char"):
  if "cp" in char.attrib:
    chars[int(char.attrib["cp"], 16)] = char

blocks = []
charToBlock = {}
for block in root.findall("{http://www.unicode.org/ns/2003/ucd/1.0}blocks/{http://www.unicode.org/ns/2003/ucd/1.0}block"):
  i = len(blocks)
  blocks.append(block)
...
for _, char in chars.items():
  ...
  docFile = cp + ".html"
  props = [
    ("Block", "<a href='%s'>%s</a>" % ("b" + block.attrib["first-cp"] + ".html", block.attrib["name"])),
  ...
  if char.attrib.get("dm", "#") != "#":
    ...
    if None in dms:
      ...
      props.append(("Decomposition", " ".join(["<a href='%s'>%s</a>" % (dm.attrib["cp"] + ".html", charTitle(dm)) for dm in dms])))
  ...
  with open(os.path.join(generatedDocsDir, docFile), "w") as f:
    f.write("<head><link rel=\"stylesheet\" href=\"../c.css\"><title>%(title)s</title></head><body class='c'><h1>%(nameOrOldName)s <small>U+%(cp)s</small></h1><figure>%(entity)s</figure><table><tbody>%(props)s</tbody></table></body>" % {
  ...
  c.execute('''INSERT OR IGNORE INTO searchIndex(name, type, path) VALUES (?, ?, ?)''', (charTitle(char), "Element", "c/" + docFile))
...
for block in blocks:
  ...
  for char in range(first, last + 1):
    if char in chars:
      ...
      overviewChars.append("<a href='%s'>%s</a>" % (chars[char].attrib["cp"] + ".html", htmlOverviewChar))
      blockChars.append("<li><i>%s</i> <a href='%s'>%s</a></i>" % (htmlChar, chars[char].attrib["cp"] + ".html", charTitle(chars[char])))
  with open(os.path.join(generatedDocsDir, docFile), "w") as f:
    f.write("<head><link rel=\"stylesheet\" href=\"../c.css\"><title>%(name)s</title></head><body class='b'><h1>%(name)s</h1><h2>Overview</h2><nav>%(overviewChars)s</nav><h2>Characters</h2><ul>%(blockChars)s</ul></body>" % {
      "name": name,
      "overviewChars": " ".join(overviewChars),
      "blockChars": "".join(blockChars),
    })
  c.execute('''INSERT OR IGNORE INTO searchIndex(name, type, path) VALUES (?, ?, ?)''', (name, "Section", "c/" + docFile))
...
""" % {
    "name": name,
    "blocks": "".join(["<li><a href='%s'>%s</a> (%s - %s)</li>" %("c/b" + b.attrib["first-cp"] + ".html", b.attrib["name"], b.attrib["first-cp"], b.attrib["last-cp"]) for b in blocks])
  })
